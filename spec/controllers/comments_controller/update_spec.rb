...
RSpec.describe Api::V1::CommentsController, type: :controller do
  ...
  describe 'POST #create' do
    ...
    let!(:correct_comment) { create(:comment, user_id: user.id) }
    let!(:invalid_comment) { build(:comment, text: nil, rating: nil, user_id: user.id) }
    ...
    context 'when user add current comment and user update current comment' do
      ...
      it 'returns json with params of updated comment' do
        patch :update, params: { id: correct_comment.id, comment: correct_comment.attributes }
      ...
      it 'returns message errors' do
        patch :update, params: { id: correct_comment.id, comment: invalid_comment.attributes }
