...
class Api::V1::AttendancesController < ApplicationController
  ...
  def create
    return head :unprocessable_entity if event.decorate.already_subscribed?(current_user.id)
  ...
  def destroy
    return head :unprocessable_entity unless event.decorate.already_subscribed?(current_user.id)
